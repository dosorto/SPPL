    ...
    <div class="min-h-screen flex items-center justify-center p-6">
        <div class="container mx-auto p-8 bg-white rounded-lg shadow-xl max-w-2xl mt-24">
            <div class="text-center mb-8">
                <h1 class="text-4xl font-extrabold text-blue-800 mb-2">Contáctanos</h1>
                <p class="text-lg text-gray-600">Estamos aquí para ayudarte. Envíanos tu consulta.</p>
            </div>

            @if (session('success'))
                <div class="bg-green-100 text-green-800 p-4 rounded-lg mb-6 text-center border border-green-200">
                    {{ session('success') }}
                </div>
            @endif
            ...
            <form action="{{ route('contacto.enviar') }}" method="POST" class="space-y-6">
                ...
                <div class="text-center">
                    <button type="submit"
                            class="w-full sm:w-auto px-6 py-3 bg-blue-700 text-white font-semibold rounded-full hover:bg-blue-600 transition-all shadow-lg text-lg">
                        Enviar Mensaje
                    </button>
                </div>
            </form>
