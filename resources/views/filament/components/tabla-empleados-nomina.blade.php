@if($empleados->isEmpty())
    <div class="text-gray-500">No hay empleados en la nómina.</div>
@else
    <!-- Aseguramos que el token CSRF está presente -->
                ...
                @foreach($empleados as $detalle)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <form method="POST" action="{{ route('filament.admin.resources.nominas.eliminar-empleado', ['nomina' => $record->id, 'detalleId' => $detalle->id]) }}" style="display:inline">
                                @csrf
                                <button 
                                    type="submit" 
                                    ...
                                    onclick="return confirm('¿Está seguro que desea eliminar este empleado de la nómina?')"
                                >
    ...
    <script>
        // Asegurar que las peticiones AJAX tienen el token CSRF
        document.addEventListener('DOMContentLoaded', function() {
            // Asegurar que el token CSRF está disponible para todas las peticiones AJAX
            ...
            if (window.Livewire) {
                window.Livewire.hook('message.failed', (message, component) => {
                    if (message.response && message.response.status === 405) {
